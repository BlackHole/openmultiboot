...
#elif defined(OMB_KERNEL_BLK)
	if (omb_utils_file_exists(OMB_PROC_STB))
		sprintf(cmd, "%s if=%s of=%s/%s/.kernels/%s.bin", OMB_DD_BIN, OMB_KERNEL_MMCBLK, OMB_MAIN_DIR, OMB_DATA_DIR, item->identifier);
#else
...
#elif defined(OMB_KERNEL_BLK)
		if (omb_utils_file_exists(OMB_PROC_STB))
			sprintf(cmd, "%s of=%s if=%s/%s/.kernels/%s.bin", OMB_DD_BIN, OMB_KERNEL_MMCBLK, OMB_MAIN_DIR, OMB_DATA_DIR, item->identifier);
#else
